		...
		leftRightContainer: $('<div>', {
			...
			uncollapseAll : function(_t) {			
				if(!$('.collapsible').length) {
					...
					$('.collapsible').on('mousedown', function(e) {
					...
					if($('.collapsible').text() === '[Expand]') {
						...
						$('.subParent .subparentarrowdownward').closest('.subParent .title').each(function(){$(this).trigger('mousedown')});
							$(this).css('display', 'none');
    ...
    var that = this,
        elements = {
            ...
            fn: function (rightClickFn, leftClickFn, tabsClickAndPostingCall) {
				...
				if($('.tabs-li').length) {
					$('.tabs-li').first().trigger('mousedown').trigger('mouseenter').trigger('touchstart');
        ...
        $('.title').on(eventName, function (e) {
            ...
            if (text in tabsNumberByKey && tabsNumberByKey[text]) {
				...
				if(document.documentElement.clientWidth < 400) {		
					$('.tabulator-wrapper').css({'width' : $('.tabs-li').first().outerWidth() + 'px'});
				}
			$('.columniser-text').first().on('mousedown', function(e) {
					...
					$('.columniser-text').children().remove();
			});
	  }else if(clicks > 1){
			 $('.columniser-text').first().css('display', 'none');	
	  }
